...
Shader "FL2D/LightBlend"
{
	...
	_Color("Tint", Color) = (1,1,1,1)
		...
		_DepthTex("Depth Buffer", 2D) = "black" {}
		_LayerTex("Camera", 2D) = "black" {}
	}
...
#include "UnityCG.cginc"
	...
	fixed4 depthColor = tex2D(_DepthTex, input.uv);
	fixed4 cameraColor = tex2D(_LayerTex, input.uv);
	//depthColor.rgb *= depthColor.a;
	...
	diffuseColor.rgb = diffuseColor.rgb * _Color.rgb * cameraColor.rgb;
	diffuseColor.rgb *= diffuseColor.a * _Color.a * cameraColor.a;
	diffuseColor *= input.intensity;
	diffuseColor /= cameraColor.a;
	diffuseColor /= cameraColor.a;
...
#include "UnityCG.cginc"
	...
	//fixed4 depthColor = tex2D(_DepthTex, input.uv);
	fixed4 cameraColor = tex2D(_LayerTex, input.uv);
	diffuseColor.rgb = _Color.rgb * diffuseColor.rgb * cameraColor.rgb * diffuseColor.a;
	...
	partColor.rgb = partColor.rgb * cameraColor.rgb * partColor.a;
	part2Color.rgb = part2Color.rgb * cameraColor.rgb * part2Color.a;

	fixed4 finalColor;
	//finalColor = diffuseColor + (partColor + part2Color) * _ParticleFactor;
	finalColor.a = _Color.a * cameraColor.a;
